---
title: "*BIO A 423:  \n  COVID-19 network effects on sexual health behavior*"
...
```{r, echo = F, fig.cap = "Survey responses to contraceptive use",fig.width=4,fig.height=2, warning = F, message = F, fig.align='left'}
...
```{r, echo = F, fig.cap = "Survey responses to demographics",fig.width=4,fig.height=2, warning = F, message = F, fig.align='left'}
# Visuals 
data_model %>% 
...
Overall, in this study I sought to understand the network effects of COVID-19 health behavior and peer group risky sexual behavior on individual contraception use or lack thereof. Broadly, I found that my results were not statistically significant for predicting whether someone decided to use contraception or not. In the logistic regression models, there was not a correlation with friend's covid safety or the respondent's living situation. However, in the multinomial model, predicting number of contraceptives used, race and gender covariates did significantly predict whether one would use no protection or some protection. Yet, due to limitations I summarize in this section, we should be skeptical of these results because of the small sample size. 

Overall, the results suggests that there is not an association between living situation and contraception use and number. Consistent with previous studies, this is not unexpected. I would have liked to include an interaction between drinking and the respondent's living status, but this was not possible due to issues of statistical power. In the multinomial model, this suggests that there is variation in the contraceptive access between different racial groups and gender identities. This may be reflective of barriers in access due to discrimination. 
